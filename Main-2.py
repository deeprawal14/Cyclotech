...
model = Model()
model.load_state_dict(torch.load('C:/Users/deepr/Desktop/group1/Cyclotech/saved_modelcyclonr', map_location=torch.device('cpu')))
model.eval()
# model ends here
...
@app.route("/")
def index():
    return render_template("index2.html", rain=1, x=0)


'''Image upload and display'''

UPLOAD_FOLDER = 'C:/Users/deepr/Desktop/group1/Cyclotech/insat3d_ir_cyclone_ds/CYCLONE_DATASET_INFRARED'
...
def predict():
    ...
    if imagefile and allowed_img(imagefile.filename):
        ...
        # execute SQL Queries
        cursor.execute(""" 
                           INSERT INTO insat_data(upload_datetime, longitude, latitude, intensity, img_date, img_time)
                            VALUES(current_timestamp,%s,%s, %s,%s,%s);""",
                       (  longitude, latitude, predicted_intensity, img_date, img_time))
        ...
        return render_template('index2.html', prediction=predicted_intensity,category=r, filename=filename, rain=r, x=1)
    else:
